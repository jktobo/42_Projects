...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/09/26 13:45:18 by dkaratae          #+#    #+#             */
/*   Updated: 2022/10/01 19:22:47 by dkaratae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	move(int x, int y, t_game *game, int key)
{
	...
	if (game->step % 2 == 0)
		ft_mlx_show_image(game, PLAYER_F, game->px + x, \
											game->py + y);
	else
	{
		if (key == 13 || key == 1)
			ft_mlx_show_image(game, PLAYER_B, game->px + x, \
												game->py + y);
		else if (key == 0)
			ft_mlx_show_image(game, PLAYER_L, game->px + x, \
												game->py + y);
		else if (key == 2)
			ft_mlx_show_image(game, PLAYER_R, game->px + x, \
												game->py + y);
	}
	game->px += x;
	game->py += y;
